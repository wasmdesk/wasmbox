...
#
# The compositor's #88 idle-repaint gate already decides WHETHER to composite a
# frame at all (idle desktop → skip). DamageSet is the next step down: when a
# paint DOES happen it records, as screen-space rectangles, exactly WHICH
# regions changed since the last composited frame, so Compositor#render can
# repaint only those regions instead of the whole canvas. It is the Ruby half of
# a dirty-rect model:
#
#   Compositor#compute_damage diffs the live scene against the last composited
#   one (@prev_wins / @prev_globals / the per-window commit seqs) and populates a
#   DamageSet; Compositor#render then either
#     - skips entirely            (empty  → idle → zero work, the #88 gate), or
#     - recomposites the whole screen (full  → first frame / resize / modal), or
#     - recomposites only the union of the damaged rectangles (draw regions),
#       clipping each so out-of-region pixels are retained from the prior frame.
#
# This extends the existing commit-seq / lastSeq damage model (the Firefox
# blit-leak fix, which already decided *whether* to re-copy an external
# surface's pixels on the JS side) to also decide *which region* of the
# compositor's own canvas to repaint on the Ruby side.
#
...
# frame/palette swap, workspace switch, an open menu, a full-screen modal). When
# too many disjoint rectangles accumulate the set also collapses to `full`,
# bounding the per-frame book-keeping.
#
