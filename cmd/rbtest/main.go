...
# RootMenu.build with a board inserts "Applets" after "Frame"; without one the
# menu is byte-for-byte the pre-applets shape (the assertions higher up already
# pin the 1-arg shape, so here we only check the 2-arg insertion).
...
assert_eq(lbls[4], "Applets", "Applets inserted at index 4 (after Frame)")
assert_eq(root_with.entries[5][:separator], true, "separator follows Applets")
assert(root_with.entries[4][:submenu].is_a?(Menu), "Applets carries a submenu")
