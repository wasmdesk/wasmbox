// Browser wasmbox external client (worker entry). Mirrors
...
const client = new WasmboxClient({ title: "Browser", w: 760, h: 500 });
...
client.start().then(async () => {
  const assets = isOCI
    ...
    : null;
  if (isOCI) {
    if (!assets) throw new Error("browser: no OCI assets envelope");
    importScripts(assets.wasm_exec_url);
  }
  const go = new Go();
  const wasmURL = assets ? assets.wasm_url : "./browser.wasm";
