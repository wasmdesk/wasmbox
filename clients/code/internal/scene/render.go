...
//	├─ (docked South, StatusBarHeight)  status  Statusbar  — Left text cells +
//	│                                                        a clickable Live
//	│                                                        Server Right segment
//	└─ (body)  HBox
...
// / tabsTheme / statusTheme / popupTheme) rather than one shared theme — the
...
// liveServerLabel is the text of the status bar's right-most (clickable)
// segment. Fixed so the Statusbar sizes that segment's hit-box once at
// construction; clicking it opens the Live Server popup (see state.go).
const liveServerLabel = "Live Server: Not connected"
...
func Paint(rgba []byte, w, h int, s *SceneState) {
	...
	// Live status-bar segments (path / Ln,Col / mode) + the active tab label,
	// both read from the live editor state each paint so the tab + status bar
	// reflect the open file and cursor without a widget rebuild.
	s.syncStatusSegments()
	s.tabs.Labels = []string{s.tabLabel()}

	s.sidebar.Draw(p, s.sidebarTheme)
	s.tabs.Draw(p, s.tabsTheme)
...
// syncStatusSegments refreshes the status bar's Left text cells from the live
// editor state: the current path, the 1-based cursor position, and the mode
// indicator. The right-most Live Server segment's text is fixed
// (liveServerLabel) and set once at construction, so it is not touched here.
func (s *SceneState) syncStatusSegments() {
	...
	s.status.Left[0].Text = path
	s.status.Left[1].Text = lnCol
	s.status.Left[2].Text = "TEXT"
}
