...
// Source / licence:
//
//	The glyphs are an original, hand-rolled 8x8 raster authored for the
//	wasmbox repository. They are deliberately simple (single-stroke, no anti-
//	aliasing) so the files browser renders text cheaply in pure Go without
//	depending on a third-party font file. As original work in this BSD-3
//	repository they carry the same BSD-3 licence as the rest of wasmbox.
//
// We deliberately keep this copy independent of clients/terminal/internal/
// scene/font.go (a sibling client) so the two browsers compose without a
// circular dependency and either one can evolve their glyph set without
// touching the other.
//
